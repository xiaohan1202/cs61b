...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int minindex = min(parentIndex(index), index);
        if (minindex == parentIndex(index) || index == 1){
            return;
        }
        else{
            swap(parentIndex(index), minindex);
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        int minindex1 = min(leftIndex(index), rightIndex(index));
        int minindex2 = min(minindex1, index);
        if (minindex2 == minindex1 && index < size - 1) {
            swap(index, minindex1);
            sink(minindex1);
        }
        else return;
        /** TODO: Your code here. */
